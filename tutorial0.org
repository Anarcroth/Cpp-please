...
machine code, thus producing an actual binary file (where there are different binary file formats such as: EFL, *.out, *COFF, SOM). This is the so called object file.
